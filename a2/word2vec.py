...
def naiveSoftmaxLossAndGradient(
    ...
    softmax_fun = softmax(np.dot(outsideVectors, centerWordVec)) #yhat
    loss = -1 * np.log(softmax_fun[outsideWordIdx])
    y = np.zeros(softmax_fun.shape[0])
    y[outsideWordIdx] = 1
    gradCenterVec = np.dot((softmax_fun - y), outsideVectors)
    gradOutsideVecs = np.outer(np.transpose(softmax_fun - y),centerWordVec)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    hold1 = sigmoid(-1 * np.dot(outsideVectors[negSampleWordIndices], centerWordVec.reshape((centerWordVec.shape[0], 1))))
    hold2 = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    loss = -1 * np.log(hold2) - np.sum(np.log(hold1), axis = 0)
    gradCenterVec = -1 * (1 - hold2) * outsideVectors[outsideWordIdx] + np.sum((1 - hold1) * outsideVectors[negSampleWordIndices], axis = 0)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    outside_hold = np.outer((1 - hold1), centerWordVec)
    for i,value in enumerate(negSampleWordIndices): gradOutsideVecs[value] += outside_hold[i]
    gradOutsideVecs[outsideWordIdx] = -(1 - hold2) * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside in outsideWords:
        each_loss , each_gradeCenter, each_gradVector = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]],word2Ind[outside],outsideVectors,dataset)
        loss += each_loss
        gradCenterVecs[word2Ind[currentCenterWord]] += each_gradeCenter
        gradOutsideVectors += each_gradVector
    ### END YOUR CODE
